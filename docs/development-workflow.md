...
### 性能门禁（BUG-0.1.5-005，长期适用）

扫描、搜索、空间布局和预览的变更需要验证数据量增加及连续跨级导航下的响应，
禁止无界保留历史树、结果和派生缓存；旧任务取消及晚响应隔离是功能契约。
性能修复记录输入规模、前后耗时和保留对象预算；原生内存按主进程、索引 helper、
WebView 渲染/GPU分别记录 Working Set 与 Private Bytes，禁止把共享页简单相加为
实际唯一物理内存。优化不得牺牲完整结果、USN增量一致性和文件操作校验。
若存在已确认的主线程卡顿或无界保留，本阶段不能仅凭功能测试通过而交付。
